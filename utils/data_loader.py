...
class data_loader:
    ...
    def load_nodes(self):
        ...
        """
        nodes = {'total':0, 'count':Counter(), 'attr':{}, 'shift':{}, 'node2type':[]}
        ...
        for i in range(len(nodes['count'].keys())):
            nodes['node2type'].append(torch.zeros(1, nodes['count'][i])+int(i))
        nodes['node2type'] = torch.transpose(torch.cat(nodes['node2type'],1), 0, 1)
